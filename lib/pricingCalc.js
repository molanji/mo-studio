...
export function calcRetainerPricing({ retainerScope, clientProfile, config }) {
  const derived = calcStudioDerived(config)

  // Point value: use manually set value, or derive from day rate
  // 1 day = 2 points (i.e. 1 point ≈ 4 hrs)
  const POINTS_PER_DAY = 2
  const pointValueRaw = derived.dailyBreakevenPerPerson > 0
    ? clientProfile.dayRate / POINTS_PER_DAY
    : 0
  const pointValue = retainerScope.point_value_inr > 0
    ? retainerScope.point_value_inr
    : Math.ceil(pointValueRaw / 500) * 500

  const pointPool = retainerScope.point_pool_monthly || 0
  const commitmentMonths = retainerScope.commitment_months || 3
  const rolloverCapPercent = retainerScope.rollover_cap_percent ?? 25
  const maxRolloverPoints = Math.floor(pointPool * rolloverCapPercent / 100)
  const freelancerBufferPercent = retainerScope.freelancer_buffer_percent || 15

  // Monthly fee waterfall
  const monthlyBaseCost = pointPool * pointValue
  const targetMarginDecimal = (config.target_margin_percent || 40) / 100
  const monthlyFeeAtMargin = targetMarginDecimal < 1
    ? monthlyBaseCost / (1 - targetMarginDecimal)
    : monthlyBaseCost
  const monthlyFeeWithBuffer = monthlyFeeAtMargin * (1 + freelancerBufferPercent / 100)
  const monthlyFixed = Math.ceil(monthlyFeeWithBuffer / 5000) * 5000

  const grossMarginPercent = monthlyFixed > 0
    ? ((monthlyFixed - monthlyBaseCost) / monthlyFixed) * 100 : 0
  const redThreshold = config.margin_red_threshold || 40
  const greenThreshold = config.margin_green_threshold || 50
  const marginStatus = grossMarginPercent < redThreshold ? 'red'
    : grossMarginPercent < greenThreshold ? 'amber' : 'green'

  const overageRatePerPoint = Math.ceil(pointValue * 1.5 / 100) * 100

  // Tax — mirrors project pricing logic
  const regionKey = clientProfile.billingRegion || 'india'
  const region = BILLING_REGIONS[regionKey] || BILLING_REGIONS.india
  const isInternational = regionKey !== 'india'
  const billingCurrency = clientProfile.billingCurrency || region.currency
  ...
  let gstMonthly = 0, totalPayableMonthly, tdsMonthly = 0, actualCashMonthly, withholdingMonthly = 0

  if (isInternational) {
    if (grossUpWithholding && region.withholdingRate > 0) {
      totalPayableMonthly = monthlyFixed / (1 - region.withholdingRate)
      withholdingMonthly = totalPayableMonthly * region.withholdingRate
      actualCashMonthly = monthlyFixed
    } else {
      totalPayableMonthly = monthlyFixed
      withholdingMonthly = monthlyFixed * region.withholdingRate
      actualCashMonthly = monthlyFixed - withholdingMonthly
    }
  } else {
    gstMonthly = monthlyFixed * region.gstRate
    totalPayableMonthly = monthlyFixed + gstMonthly
    tdsMonthly = monthlyFixed * region.tdsRate
    actualCashMonthly = totalPayableMonthly - tdsMonthly
  }

  const round100 = n => Math.round((n || 0) / 100) * 100
  const toForeign = n => exchangeRateToInr > 0 ? n / exchangeRateToInr : n

  const tpm = round100(totalPayableMonthly)
  const acm = round100(actualCashMonthly)
  ...
  return {
    ...
    rolloverCapPercent,
    maxRolloverPoints,
    freelancerBufferPercent,
    monthlyBaseCost: round100(monthlyBaseCost),
    monthlyFeeAtMargin: round100(monthlyFeeAtMargin),
    monthlyFeeWithBuffer: round100(monthlyFeeWithBuffer),
    monthlyFixed,
    grossMarginPercent: Math.round(grossMarginPercent * 10) / 10,
    marginStatus,
    overageRatePerPoint,
    ...
    withholdingRate: region.withholdingRate,
    grossUpWithholding,
    gstMonthly: round100(gstMonthly),
    tdsMonthly: round100(tdsMonthly),
    withholdingMonthly: round100(withholdingMonthly),
    totalPayableMonthly: tpm,
    actualCashMonthly: acm,
    // Contract totals
    totalContractValue: monthlyFixed * commitmentMonths,
    totalPayableContract: tpm * commitmentMonths,
    actualCashTotal: acm * commitmentMonths,
    // Foreign currency display
    monthlyFixedForeign: isInternational ? Math.round(toForeign(monthlyFixed)) : null,
    totalPayableMonthlyForeign: isInternational ? Math.round(toForeign(tpm)) : null,
    actualCashMonthlyForeign: isInternational ? Math.round(toForeign(acm)) : null,
  }
}
