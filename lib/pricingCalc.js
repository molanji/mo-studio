...
export function calcScope(scope, dayRate) {
  let totalDays = 0
  const included = []
  const excluded = []
  ...
  for (const d of DELIVERABLES) {
    ...
    if (item?.selected) {
      const qty = d.perUnit ? Number(item.quantity || 1) : 1
      const days = Number(item.days ?? d.defaultDays) * qty
      totalDays += days
      included.push({ label: d.label, days, qty: d.perUnit ? qty : null })
    } else {
      excluded.push(d.label)
    }
  }

  if (scope.other_deliverable_text && scope.other_deliverable_days) {
    totalDays += Number(scope.other_deliverable_days)
    included.push({ label: scope.other_deliverable_text, days: Number(scope.other_deliverable_days) })
  }

  const totalHours = totalDays * 8

  const thirdPartyCostsTotal =
    Number(scope.third_party?.stock_assets || 0) +
    Number(scope.third_party?.plugins || 0) +
    Number(scope.third_party?.printing || 0) +
    Number(scope.third_party?.other || 0)
